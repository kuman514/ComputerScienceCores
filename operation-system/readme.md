...
## 데드락, 교착 상태 (Deadlock)
- 둘 이상의 프로세스가 하나의 자원을 서로 점유하여 무한 대기 상태에 빠지는 현상.
- 데드락의 발생 조건
  - 비선점: 기존 프로세스가 점유하고 있던 자원을 중간에 강제로 끊어 자원을 점유할 수 없다.
  - 순환 대기 (환형 대기라고도 한다): 순환 형태로 자원을 대기하고 있어야 한다.
  - 상호 배제: 한 번에 프로세스 하나만 자원을 점유할 수 있다. 다른 프로세스가 자원을 점유하려면 기존 프로세스가 자원 점유가 끝날 때까지 기다려야 한다.
  - 점유 대기: 자원을 최소한 하나 보유하고 있므며, 기존에 다른 프로세스가 이미 점유하고 있는 자원을 점유하기 위한 대기 프로세스가 있어야 한다.
