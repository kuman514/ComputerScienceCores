...
## 그리디 알고리즘
- 전체 해를 고려하지 않고, 지금 당장의 최선해만 고르는 알고리즘.
- 선택의 순간마다 지역적인 최선해만을 골라온 과정의 최종해가 곧 최선해가 되는 문제에 적합하다.
- 계산이 빠르고 효율적이라는 장점도 있지만, 나중이 아닌 지금 당장만을 생각하기 때문에 최종해가 최선해가 되는 것을 보장해줄 수 없다는 단점도 있다.
- 그리디 알고리즘이 유효한 조건
  - 탐욕적 선택 속성: 앞의 선택이 이후 선택에 영향을 주어선 안 됨. 즉, 지금 최선의 선택이 전체 최선의 선택으로 가는 길어야 한다.
  - 최적 부분 구조: 문제에 대한 최종 최선해가 부문 문제의 최선해들로 구성될 수 있어야 한다.
