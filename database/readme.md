...
- 정규화되지 않은 데이터베이스에서 발생하는 문제점
  - 삽입 이상: 데이터를 삽입할 때 불필요한 데이터까지 입력해야 하는 경우.
  - 삭제 이상: 데이터를 삭제할 때 유지되어야 할 다른 데이터까지 삭제해야 하는 경우.
  - 갱신 이상: 데이터가 일부만 변경되어 데이터 불일치가 발생하는 경우.
...
- 인덱스 구현 예시
  - B-Tree
    ...
    - 구조적 특징
      - 노드마다 2개 이상의 키-데이터 쌍이 들어가며, 노드 내에서 항상 정렬되어 저장된다.
      - `M`차 B-Tree(즉, 트리의 차수가 `M`인 B-Tree)의 각 노드는 `ceil(M / 2)` ~ `M`개의 자식 노드를 가질 수 있다.
      - `M`차 B-Tree의 각 노드는 `ceil(M / 2) - 1` ~ `M - 1`개의 키-데이터 쌍을 가질 수 있다.
      - 어떤 노드의 키-데이터 쌍이 `k`개라면, 그 노드의 자식 노드는 `k + 1`개여야 한다.
      - 어떤 노드 내 키보다 작은 키는 그 노드의 왼쪽 자식 노드에, 큰 키는 그 노드의 오른쪽 자식 노드에 저장된다.
      - 모든 리프 노드들은 서로 동일한 레벨에 존재한다.
    - 참고: https://code-lab1.tistory.com/217
  - B+Tree
    - B-Tree를 개선한 구조.
    - 실제로 대부분의 DBMS에서 사용되는 인덱스는 이 자료구조를 사용하고 있다.
    - 구조적 특징 (B-Tree와 비교하여 어떤 점이 달라졌는지?)
      - 모든 데이터가 리프 노드에 있다. 즉, B-Tree에선 모든 노드가 키-데이터를 가지고 있었다면, B+Tree에선 비 리프 노드는 키만 가지고 있고 리프 노드는 키와 데이터를 모두 가지고 있다.
      - 모든 리프노드가 링크드 리스트의 형태를 띄고 있다. 즉, 옆에 있는 리프 노드를 검사하고자 할 때, B-Tree에선 루트 노드부터 다시 검사해야 했다면, B+Tree는 리프 노드를 통해 선형 검사를 할 수 있어 시간복잡도가 굉장히 줄어든다.
      - 왼쪽 리프 노드의 키는 오른쪽 리프 노드의 키보다 작거나 같다.
      - 모든 노드에 데이터를 저장하는 B-Tree와 달리 B+Tree는 리프 노드에만 데이터를 저장하기 때문에, 메모리 효율이 좋다.
